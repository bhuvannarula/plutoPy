...
class arucoCamera:
    ...
    def update(self, state : arucoState, target_ID : int, targetAngle : lowPassFilter):
        if (target_ID in self.coord_data):
            state.update(self.coord_data[target_ID])

            if self.debug : print("Coordinates", self.coord_data[target_ID])

            tar_rot_mtx = cv.Rodrigues(self.rvec[target_ID])[0].reshape((3,3))
            rets = cv.RQDecomp3x3(tar_rot_mtx)
            targetAngle.update(rets[0][2] + 180) 
